...
var debugOverlay = new ol.layer.Vector({
    zIndex: 9999,
    map: map,
    source: new ol.source.Vector({
    ...
    style: function(feature) {
        var label = feature.get('debugLotNumber');
        if (!label) {
            return null;
        }
        return new ol.style.Style({
            ...
            geometry: function(feat) {
                ...
                if (g && typeof g.getInteriorPoint === 'function') {
                    try { return g.getInteriorPoint(); } catch (e) {}
                }
                return g;
            }
        });
...
function cloneTextStyleWithOpacity(textStyle, opacity) {
    if (!textStyle || typeof textStyle.clone !== 'function') {
        return null;
    }
    var clone = textStyle.clone();
    if (clone.getFill && clone.getFill() && ol.color && typeof ol.color.asArray === 'function') {
        var rgba = ol.color.asArray(clone.getFill().getColor());
        if (rgba && rgba.length === 4) {
            rgba[3] = opacity;
            clone.getFill().setColor(rgba);
        }
    }
    return clone;
}
...
function onPointerMove(evt) {
	...
	if (doHighlight) {
        if (currentFeature !== highlight) {
            if (highlightClone) {
                featureOverlay.getSource().removeFeature(highlightClone);
                highlightClone = null;
            }
            if (debugHighlight) {
                debugOverlay.getSource().removeFeature(debugHighlight);
                debugHighlight = null;
            }
            if (currentFeature) {
                var featureStyle;
                var featureTextStyle = null;
                if (typeof clusteredFeatures == "undefined") {
				var style = currentLayer.getStyle();
				var styleFunction = typeof style === 'function' ? style : function() { return style; };
				var styleArray = styleFunction(currentFeature);
				if (Array.isArray(styleArray) && styleArray.length) {
					featureStyle = styleArray[0];
					for (var si = 0; si < styleArray.length; si++) {
						if (!featureTextStyle && typeof styleArray[si].getText === 'function' && styleArray[si].getText()) {
							featureTextStyle = styleArray[si].getText();
						}
						if (!featureStyle && typeof styleArray[si].getStroke === 'function' && styleArray[si].getStroke()) {
							featureStyle = styleArray[si];
						}
					}
					if (!featureTextStyle && featureStyle && typeof featureStyle.getText === 'function') {
						featureTextStyle = featureStyle.getText();
					}
				} else if (styleArray) {
					featureStyle = styleArray;
					if (typeof featureStyle.getText === 'function') {
						featureTextStyle = featureStyle.getText();
					}
				}
			} else {
                ...
                var highlightText = cloneTextStyleWithOpacity(
                    featureTextStyle,
                    0.7
                );
                // If there is no cloned text style (label not produced by the generated style),
                // create a fallback text style using common cadastral fields so numbers always show.
                if (!highlightText) {
                    var candidateFields = [
                        "PIN",
                        "LOT NUMBER",
                    ...
                    var pinFallback = null;
                    for (var i = 0; i < candidateFields.length; i++) {
                        var fieldName = candidateFields[i];
                        if (currentFeature.get(fieldName) != null) {
                            pinFallback = currentFeature.get(fieldName);
                            break;
                        }
                    }
                    if (!pinFallback) {
                        var featureKeys = currentFeature.getKeys();
                        for (var j = 0; j < featureKeys.length; j++) {
                            var key = featureKeys[j];
                            if (/pin|lot|parcel|parcel_no|lotno|lot_number|number/i.test(key) && currentFeature.get(key) != null) {
                                pinFallback = currentFeature.get(key);
                                break;
                            }
                        }
                    }
                    if (pinFallback) {
                        highlightText = new ol.style.Text({
                            font: "9.1px 'Segoe UI', sans-serif",
                            text: String(pinFallback),
                            textBaseline: "middle",
                            textAlign: "left",
                            offsetX: 8,
                            offsetY: 3,
                            placement: 'point',
                            maxAngle: 0,
                            fill: new ol.style.Fill({ color: 'rgba(236,0,0,0.7)' }),
                            stroke: new ol.style.Stroke({ color: '#f4f4f4', width: 3 })
                        });
                    }
                }
                var highlightStroke = featureStyle && typeof featureStyle.getStroke === 'function' ? featureStyle.getStroke() : null;
                ...
                if (currentFeature.getGeometry().getType() == 'Point' || currentFeature.getGeometry().getType() == 'MultiPoint') {
                    ...
                    var highlightFillStyle = new ol.style.Style({
                    ...
                    var highlightTextStyle = new ol.style.Style({
                        text: highlightText,
                        zIndex: 2000,
                        geometry: function(feat) {
                            var g = feat.getGeometry();
                    ...
                    highlightClone = currentFeature.clone();
                    featureOverlay.getSource().addFeature(highlightClone);
                    featureOverlay.setStyle(function(f) {
                        return [highlightFillStyle, highlightTextStyle];
                    });

                    var forcedLotNumber = currentFeature.get('LOT NUMBER') || currentFeature.get('PIN') || currentFeature.get('LOT') || currentFeature.get('Lot Number') || currentFeature.get('lot_number');
                    if (forcedLotNumber != null) {
                        debugHighlight = currentFeature.clone();
                        debugHighlight.set('debugLotNumber', String(forcedLotNumber));
                        debugOverlay.getSource().addFeature(debugHighlight);
                    }
