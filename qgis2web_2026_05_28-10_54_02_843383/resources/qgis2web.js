...
map.getView().fit([13756334.338196, 1152710.399516, 13777487.109330, 1163678.987266], map.getSize());
...
// Print functionality
var selectedFeatureData = null;
var selectedFeatureProperties = null;

// Global variable to store feature data for printing - ensure it's accessible to parent window
if (typeof window.selectedFeatureDataForPrint === 'undefined') {
...
function updateCadastralInfo(feature, layer) {
    ...
    if (!keys.length) {
        target.innerHTML = '<p class="hint">This feature has no visible cadastral attributes.</p>';
        window.selectedFeatureDataForPrint = null;
        return;
    }
    ...
    // Store the selected feature data for printing - use window object for global access
...
function openPrintWindow(featureProperties, mapImage) {
    var printUrl = '../print.html';
    var printWindow = window.open(printUrl, 'print-window', 'width=1400,height=1000,scrollbars=yes');
    
    // Wait for the print window to load and then populate it with data
    if (printWindow) {
        printWindow.addEventListener('load', function() {
            try {
                ...
                if (mapContainer) {
                    var existingImg = mapContainer.querySelector('img');
                    var existingDiv = mapContainer.querySelector('div');
                    ...
                    if (existingImg) {
                        mapContainer.removeChild(existingImg);
                    }
                    if (existingDiv) {
                ...
                if (featureProperties) {
                    var certText = printWindow.document.querySelector('.cert-text');
                    if (certText) {
                        // Extract key information from feature properties
                        var lotNo = featureProperties['LOT_NO'] || featureProperties['lot'] || 'N/A';
                        var barangay = featureProperties['BARANGAY'] || featureProperties['brgy'] || 'N/A';
                        var area = featureProperties['AREA'] || featureProperties['area'] || 'N/A';
                        var pin = featureProperties['PIN'] || featureProperties['pin'] || 'N/A';
                        var owner = featureProperties['OWNER'] || featureProperties['owner'] || 'N/A';
                        var dateIssued = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
                        
                        // Update the main certificate text with dynamic values
                        var paragraphText = 'Accompanying is a certified Sketch Plan of Lot No. <strong>' + 
                            lotNo + '</strong> Toledo Cadastre, situated at Barangay <strong>' + 
                            barangay + '</strong>, Toledo City containing an area of <strong>' + 
                            area + '</strong> square meters identified in the Tax Mapping Roll as <strong>' + 
                            pin + '</strong> in the name of <strong>' + 
                            owner + '</strong>.';
                        
                        var requestText = 'Issued Upon request of <strong>' + owner + '</strong> on <strong>' + dateIssued + '</strong>';
                        
                        // Find and update paragraphs
                        var paragraphs = certText.querySelectorAll('p');
                        if (paragraphs[0]) {
                            paragraphs[0].innerHTML = paragraphText;
                        }
                        if (paragraphs[1]) {
                            paragraphs[1].innerHTML = requestText;
                        }
                    }
                }
            } catch (error) {
...
function handlePrintClick() {
    if (selectedFeatureProperties) {
        captureMapAsImage().then(function(mapImage) {
            openPrintWindow(selectedFeatureProperties, mapImage);
        }).catch(function(error) {
            console.error('Error capturing map:', error);
            alert('Error preparing map for print. Please try again.');
        });
    } else {
        alert('No feature selected. Please select a land feature first.');
    }
}

// Add print button click handler
var printButton = document.getElementById('popup-print');
if (printButton) {
    printButton.addEventListener('click', handlePrintClick);
} else {
    console.warn('Print button not found in DOM');
    // Fallback: Try to find and attach the event listener after a short delay
    setTimeout(function() {
        var printBtn = document.getElementById('popup-print');
        if (printBtn) {
...
function onSingleClickFeatures(evt) {
    ...
    if (popupText === '<ul>') {
        ...
        window.selectedFeatureDataForPrint = null;
        // Hide cadastral panel when no feature is selected
        var panel = document.querySelector('.cadastral-panel');
        if (panel) {
        ...
        popupText += '</ul>';
        // Store the last selected feature's properties for printing
        if (currentFeature) {
            selectedFeatureProperties = currentFeature.getProperties();
            // Update cadastral panel
            if (currentLayer) {
                updateCadastralInfo(currentFeature, currentLayer);
            }
        }
    }
